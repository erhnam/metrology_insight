...
fn calculate_signal_power(
    ...
    if d_length != 0.0 {
        ...
        // Validación de límites ANTES de acceder al índice last_index + 1
        if last_index + 1 >= signal1.len() || last_index + 1 >= signal2.len() {
            return 0.0; // o podrías continuar sin añadir esta muestra
        }

        let ysample1 =
            signal1[last_index] as f64 + (signal1[last_index + 1] as f64 - signal1[last_index] as f64) * d_length;
        let ysample2 =
            signal2[last_index] as f64 + (signal2[last_index + 1] as f64 - signal2[last_index] as f64) * d_length;
...
pub fn calculate_phase_angle_from_signal_values(
    signal1: &[i32],
    signal2: &[i32],
    freq_zc: f64,
    length: usize,
