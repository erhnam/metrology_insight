...
const ZERO_CROSSING_MAX_POINTS: usize = 5; // Maximum number of zero crossing points to store // Para 1 ciclo, 2 cruces por cero (ascendente + descendente)

const EXTRA_SAMPLES: usize = 0; /* Extra samples to a cycle to get zero crossing */
...
pub fn calculate_zero_crossing_freq(signal: &[i32], adc_samples_second: f64) -> f64 {
    ...
    for p in 0..(num_samples - 1) {
        ...
        if debounce == 0 && signal[p] < 0 && signal[p + 1] >= 0 {
            // Interpolación
            let x1 = p as f64;
            let x2 = (p + 1) as f64;
            ...
            if (y2f - y1f).abs() > f64::EPSILON {
                let xp = x1 + (0.0 - y1f) * (x2 - x1) / (y2f - y1f);
...
fn signal_integrate(
    ...
    length: usize,
    freq_zc: f64,
    adc_currents_d2a_factor: f64,
    adc_samples_second: f64,
) {
    let mut integral: f64 = 0.0;
    let mut integrated_signal: Vec<i32> = Vec::new();
    let orms: f64 =
        voltage_current::calculate_rms(signal, length, freq_zc, adc_samples_second) / adc_currents_d2a_factor;

    // Cumulative integration by trapezoid rule
    for i in 0..signal.len() {
        let y_x: f64 = signal[i] as f64;
        let y_x1: f64 = if i + 1 < signal.len() {
            signal[i + 1] as f64
        } else {
            y_x
        };
    ...
    let integral_rms: f64 = voltage_current::calculate_rms(&integrated_signal, length, freq_zc, adc_samples_second)
        / adc_currents_d2a_factor;
...
pub fn process_signal(
    ...
    if !signal.signal.is_empty() && signal.length > 0 {
        ...
        signal.freq_nominal =
            calculate_signal_frequency_nominal(m_signal.freq_zc, &mut signal.length, signal.freq_nominal);
        ...
        // RMS calculation
        m_signal.rms = voltage_current::calculate_rms(
            &signal.signal,
            signal.length_cycle,
            m_signal.freq_zc,
            adc_samples_second,
