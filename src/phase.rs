...
fn phase_angle_from_signals(voltage: &[f32], current: &[f32]) -> f32 {
    ...
    let v_mag: f32 = voltage.iter().map(|v| v * v).sum::<f32>().sqrt();
    let i_mag: f32 = current.iter().map(|i| i * i).sum::<f32>().sqrt();
...
fn absolute_phase_angles_from_signals(voltage_signal: &[f32], current_signal: &[f32], adc_samples_second: f32, freq_est: f32) -> (f32, f32) {
...
fn all_phase_angles_from_signals(voltage_signal: &[f32], current_signal: &[f32], adc_samples_second: f32, freq_est: f32) -> PhaseAngleMetrics {
    let (v_angle, c_angle) = absolute_phase_angles_from_signals(voltage_signal, current_signal, adc_samples_second, freq_est);
...
pub fn update_phase_angles(socket: &mut MetrologyInsightSocket, adc_samples_second: f32, active_phases: usize) {
    for i in 0..active_phases {
        let freq_est = socket.phases[i].voltage.pll_state.freq_est;
        if freq_est <= 0.0 { continue; }
...
mod tests {
    ...
    fn test_phase_direction_inductive_capacitive_inphase() {
        ...
        // In-phase signals
        let v_in: Vec<f32> = (0..n_samples).map(|i| (2.0 * PI * freq * (i as f32 / fs)).sin()).collect();
        let i_in: Vec<f32> = (0..n_samples).map(|i| (2.0 * PI * freq * (i as f32 / fs)).sin()).collect();
        ...
        let i_ind: Vec<f32> = (0..n_samples).map(|i| (2.0 * PI * freq * (i as f32 / fs) - 30.0 * PI / 180.0).sin()).collect();
        ...
        let i_cap: Vec<f32> = (0..n_samples).map(|i| (2.0 * PI * freq * (i as f32 / fs) + 30.0 * PI / 180.0).sin()).collect();
